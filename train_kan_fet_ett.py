...
def load_ett_csv(csv_path: str, target_col: str = "OT"):
    df = pd.read_csv(csv_path)

    # Keep time ordering (optional but nice)
...
def forecast_trend_plot(
    ...
    plt.legend()
    plt.savefig(f"ETT_plots/ETTh1/horizon8/{plot_name}", bbox_inches="tight")
    plt.show()
...
def train_and_predict(cfg: TrainConfig):
    ...
    df_raw, df_num, X_raw, y_raw = load_ett_csv(cfg.csv_path, cfg.target_col)
    N, F = X_raw.shape
    ...
    forecast_trend_plot(
        plot_name="MLP-Style Neural ODE ETTh1 Forecast vs ground truths",
        model=model,
...
def train_and_predict_diffusion(cfg: TrainConfig):
    ...
    forecast_trend_plot(
        plot_name="Diffusion Neural ODE ETTh1 Forecast vs ground truths",
        model=model,
        X=X,                    # standardized X (NOT X_raw)
        y_raw=y_raw,            # original target units
        context_len=cfg.context_len,
...
def train_and_predict_kan_diffusion(cfg: TrainConfig):
    ...
    with torch.no_grad():
        z0 = model.encoder(x_ctx)
        #this becomes incredibly slow for large context lengths
        z_traj = odeint(model.dynamics, z0, t_fut, method="dopri5")  # (T,B,D)
        cond = model._make_cond(z_traj)
        y_hat_std = diffusion.p_sample_loop(model.eps_head, (1, cfg.pred_len), cond, device).squeeze(0).cpu().numpy()
    y_hat = y_hat_std * y_sd[0] + y_mu[0]
    ...
    forecast_trend_plot(
        plot_name="KAN Diffusion Neural ODE ETTh1 Forecast vs ground truths",
        model=model,
        X=X,                    # standardized X (NOT X_raw)
        y_raw=y_raw,            # original target units
        context_len=cfg.context_len,
...
def train_and_predict_kan_fet_diffusion(cfg: TrainConfig):
    ...
    forecast_trend_plot(
        plot_name="KAN-FET Diffusion Neural ODE ETTh1 Forecast vs ground truths",
        model=model,
        X=X,                    # standardized X (NOT X_raw)
        y_raw=y_raw,            # original target units
        context_len=cfg.context_len,
...
if __name__ == "__main__":
    ...
    cfg = TrainConfig(
    model=LatentNeuralODEForecaster(num_features=7, context_len=96, pred_len=24),
    csv_path="data/ETT/ETTh1.csv",
    target_col="OT",
    context_len=96,
    pred_len=8,
    batch_size=64,
