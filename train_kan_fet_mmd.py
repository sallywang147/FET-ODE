...
def forecast_trend_plot(
    ...
    device,
    rk4_substeps=4,
    horizon=1,         # 0 = first step of the pred_len forecast
    ...
    plt.plot(pred_series, label=f"forecast (horizon={horizon})")
    ...
    plt.ylabel("Gas Price  (Dollar per Gallon)")
    plt.legend()
    plt.show()
...
class TrainConfig:
    model: nn.Module
    csv_path: str = "Time_MMD/numerical/Energy/Energy.csv"
    target_col: str = "OT"
    context_len: int = 32
    pred_len: int = 8
    batch_size: int = 64
...
def load_energy_csv(csv_path: str, target_col: str):
    df = pd.read_csv(csv_path)

    # Parse date if present (not used by default; keeps ordering)
    if "date" in df.columns:
        df["date"] = pd.to_datetime(df["date"], errors="coerce")

    # Keep only numeric columns for features
    # (drop date/start_date/end_date strings if they exist)
    ...
    # Features = all numeric columns (including target is OK; model can use it)
    X = numeric_df.values  # (N,F)
    y = numeric_df[target_col].values  # (N,)
...
def train_and_predict(cfg: TrainConfig):
    ...
    df_raw, df_num, X_raw, y_raw = load_energy_csv(cfg.csv_path, cfg.target_col)
    N, F = X_raw.shape
    ...
    forecast_trend_plot(
        plot_name="MLP-Style Neural ODE Energy Forecast vs ground truths",
        model=model,
...
def train_and_predict_diffusion(cfg: TrainConfig):
    ...
    forecast_trend_plot(
        plot_name="Diffusion Neural ODE Energy Forecast vs ground truths",
        model=model,
        X=X,                    # standardized X (NOT X_raw)
        y_raw=y_raw,            # original target units
        context_len=cfg.context_len,
...
def train_and_predict_kan_diffusion(cfg: TrainConfig):
    ...
    forecast_trend_plot(
        plot_name="KAN Diffusion Neural ODE Energy Forecast vs ground truths",
        model=model,
        X=X,                    # standardized X (NOT X_raw)
        y_raw=y_raw,            # original target units
        context_len=cfg.context_len,
...
def train_and_predict_kan_fet_diffusion(cfg: TrainConfig):
    ...
    forecast_trend_plot(
        plot_name="KAN-FET Diffusion Neural ODE Energy Forecast vs ground truths",
        model=model,
        X=X,                    # standardized X (NOT X_raw)
        y_raw=y_raw,            # original target units
        context_len=cfg.context_len,
...
if __name__ == "__main__":
    ...
    cfg = TrainConfig(
        model= LatentNeuralODEForecaster(
            num_features=9,          # Energy.csv has 5 numeric columns
            context_len=50,
            pred_len=8,
            latent_dim=64,
        ).to("cuda" if torch.cuda.is_available() else "cpu"),
        csv_path="Time_MMD/numerical/Energy/Energy.csv",
        target_col="Weekly East Coast All Grades All Formulations Retail Gasoline Prices  (Dollars per Gallon)",
        context_len=50,
        pred_len=8,
        batch_size=48,
        epochs=50,
