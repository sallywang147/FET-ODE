...
def plotter(plot_name, pred, soln_arr, epoch, loss_train, loss_test):
    ...
    plt.legend(['x_data', 'y_data', 'x_KanFet_MLP_NODE', 'y_KanFet_MLP_NODE'])
...
for epoch in tqdm(range(num_epochs)):
    ...
    if loss_train<loss_min:
        ...
        if opt_plot_counter>=200:
            print('plotting optimal model')
            kan_plotter_opt("kan_fet_mlp_node_plots", pred_test[:, 0, :], soln_arr, epoch, kan_model_loss_list_train, kan_model_loss_list_test)
            opt_plot_counter=0
    ...
    if epoch % plot_freq ==0:
        #model.save_ckpt('ckpt_predprey')
        kan_plotter("kan_fet_mlp_node_plots", pred_test[:, 0, :], soln_arr, epoch, loss_list_train, loss_list_test)
